DROP PROCEDURE IF EXISTS GetConfirmedItineraryPlans;
DELIMITER //

CREATE PROCEDURE GetConfirmedItineraryPlans(
    IN input_offset INT,
    IN input_limit INT,
...
BEGIN
    SET @query = CONCAT(
        ...
        "dip.itinerary_quote_ID AS itinerary_booking_id, ",
        "ipd.itinerary_quote_ID AS itinerary_quote_id, ",
        "dip.no_of_routes, ",
        "dip.no_of_days, ",
        "dip.no_of_nights, ",
        "dip.total_adult, ",
        ...
        "dip.deleted, ",
        "dip.createdon, ",
         "dip.itinerary_cancellation_status, ",
        ...
        "JOIN dvi_itinerary_plan_details ipd ON dip.itinerary_plan_ID = ipd.itinerary_plan_ID ",
        ...
        "WHERE dip.deleted = '0' AND ipd.deleted = '0' "
    );
    ...
    IF include_cancelled = 1 THEN
        SET @query = CONCAT(@query,
            " AND EXISTS ( ",
            "   SELECT 1 ",
            "   FROM dvi_cancelled_itineraries dci ",
            "   WHERE dci.itinerary_plan_id = dip.itinerary_plan_ID ",
            "   AND dci.status = 1 ",
            "   AND dci.deleted = 0 ",
            ") "
    ...
    IF search_value IS NOT NULL AND search_value <> '' THEN
        SET @query = CONCAT(@query,
            ...
            "OR ipd.itinerary_quote_ID LIKE '%", search_value, "%' ",
            "OR du.username LIKE '%", search_value, "%')"
        );
    END IF;

    IF start_date IS NOT NULL AND start_date <> '' AND start_date <> '0000-00-00' THEN
        SET @query = CONCAT(@query, " AND DATE(dip.trip_start_date_and_time) = '", start_date, "'");
    END IF;

    IF end_date IS NOT NULL AND end_date <> '' AND end_date <> '0000-00-00' THEN
    ...
    IF filter_guide_id IS NOT NULL AND filter_guide_id <> 0 THEN
        SET @query = CONCAT(@query,
            " AND EXISTS ( ",
            "    SELECT 1 ",
            "    FROM dvi_itinerary_route_guide_details gd ",
            "    WHERE gd.itinerary_plan_ID = dip.itinerary_plan_ID ",
            "    AND gd.guide_id = ", filter_guide_id,
            "    AND gd.status = 1 ",
            ") "
        );
    END IF;
