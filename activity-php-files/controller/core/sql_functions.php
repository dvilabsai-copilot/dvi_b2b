...
function getITINERARY_HOTSPOT_VEHICLE_PARKING_CHARGES_DETAILS($vehicle_type_id, $itinerary_plan_ID, $itinerary_route_ID, $requesttype)
{
	if ($requesttype == 'total_hotspot_parking_charges') :
		...
		// Fallback: if cached parking rows are missing, calculate from distinct route hotspots + parking pricebook.
		if (empty($parking_charges_amt) || $parking_charges_amt <= 0) :
			$fallback_parking_query = sqlQUERY_LABEL("SELECT COALESCE(SUM(P.`parking_charge`), 0) AS TOTAL_HOTSPOT_PARKING_CHARGES FROM (SELECT DISTINCT `hotspot_ID` FROM `dvi_itinerary_route_hotspot_details` WHERE `deleted` = '0' AND `status` = '1' AND `itinerary_plan_ID` = '$itinerary_plan_ID' AND `itinerary_route_ID` = '$itinerary_route_ID' AND `hotspot_ID` > 0) H LEFT JOIN `dvi_hotspot_vehicle_parking_charges` P ON P.`hotspot_id` = H.`hotspot_ID` AND P.`vehicle_type_id` = '$vehicle_type_id' AND P.`deleted` = '0' AND P.`status` = '1'") or die("#1-UNABLE_TO_COLLECT_PARKING_CHARGE_DETAILS_LIST:" . sqlERROR_LABEL());
			while ($fetch_fallback_parking_data = sqlFETCHARRAY_LABEL($fallback_parking_query)) :
				$parking_charges_amt = $fetch_fallback_parking_data['TOTAL_HOTSPOT_PARKING_CHARGES'];
			endwhile;
		endif;
